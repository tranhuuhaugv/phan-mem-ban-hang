...
import { Button, PageHeader, Table, Tr, Td, Input, Field, SearchInput } from "@/components/ui";
    ...
    <div>
      ...
      <Table head={["Tên", "SĐT", "Địa chỉ", "Số đơn", "Tổng mua", ""]}>
        {rows.map((c) => (
          <Tr key={c.id}>
            <Td>
              <Link href={`/khach-hang/${c.id}`} className="font-medium hover:text-[var(--primary)] hover:underline">
                {c.name}
              </Link>
